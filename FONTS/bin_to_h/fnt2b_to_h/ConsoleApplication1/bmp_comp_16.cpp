...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (bwidth > 8000 || bheight > 4000)
	{
		printf("Image size is too big: %ux%u pix", bwidth, bheight);
		return 1;
	}
	WORD	wwidth = bwidth;
	...
	_tcscpy(wfname, argv[1]);
	if((cd = _tcschr(wfname, '.')) > 0)
	{
		cd[0] = 0;
	}
	_tcscat(wfname, _T(".cimg1"));
	...
	imgbase = (WORD*)(bmpdata + inpos);
	endimgbase = (WORD*)(bmpdata + fsize);
...
int process_comp()
{
     ...
     while (get_word() != EOF)
        ...
        if (ch != tmpbuf[1])
            ...
            WriteFile(wfile, &(tmpbuf[1]), 2, &wr, NULL);
 ...
 *----------------------------------------------------------------*/
void process_uncomp(WORD char1)
{
...
void flush_outbuf()
{
    ...
    for ( ; outbuf_cnt; outbuf_cnt--)
		WriteFile(wfile, &(outbuf[pos++]), 2, &wr, NULL);
}
