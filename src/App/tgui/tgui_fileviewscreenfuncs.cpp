...
void		_tgui_FileviewFileinfoPaint(void *tguiobj, void *param)
{
	...
	LCDUI_FONT_TYPE 	oldfont;

	if (LCDUI_GetScreenWidth() == 480)
	{
		oldfont = LCDUI_SetFont(LCDUI_FONT_H12BOLD);
	} else
	if (LCDUI_GetScreenWidth() == 320)
	{
		oldfont = LCDUI_SetFont(LCDUI_FONT_H11);
	}
	...
	if (LCDUI_GetScreenWidth() == 320)
	{
		LCDUI_FONT_TYPE oldfont = LCDUI_SetFont(LCDUI_FONT_H11BOLD);
	}
...
void		_tgui_FileviewPrintinfoPaint(void *tguiobj, void *param)
{
	...
	if ((PFILE_GetTotalLayers() * PFILE_GetLayerThickness()) > (cfgMotor.max_pos - PFILE_GetLiftHeight()))
		...
		LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_TOTAL_LAYERS), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	if ((PFILE_GetTotalLayers() * PFILE_GetLayerThickness()) > (cfgMotor.max_pos - PFILE_GetLiftHeight()))
		LCDUI_SetColor(LCDUI_RGB(0xE00000));
	else
		LCDUI_SetColor(LCDUI_RGB(0x00496C));
	sprintf(msg, "%u", PFILE_GetTotalLayers());
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// print time
	ytop += fntheight + yinc;
	uint32_t	ptime = (uint32_t)PFILE_GetPrintTime();
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_PRINT_TIME), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	...
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_LAYER_THICKNESS), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	sprintf(msg, "%0.3f %s", PFILE_GetLayerThickness(), LANG_GetString(LSTR_SHORT_MILLIMETERS));
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// antialiasing
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_ANTIALIASING), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	...
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_TIME_LIGHT), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	if (LCDUI_GetScreenWidth() == 480)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetLightLayer(), LANG_GetString(LSTR_SHORT_SECOND));
	} else
	if (LCDUI_GetScreenWidth() == 320)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetLightLayer(), LANG_GetString(LSTR_SHORTEST_SECOND));
	}
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// bottom layer light
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_TIME_BOTTOM_LIGHT), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	if (LCDUI_GetScreenWidth() == 480)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetLightBottom(), LANG_GetString(LSTR_SHORT_SECOND));
	} else
	if (LCDUI_GetScreenWidth() == 320)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetLightBottom(), LANG_GetString(LSTR_SHORTEST_SECOND));
	}
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// light pause
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_TIME_PAUSE_LIGHT), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	if (LCDUI_GetScreenWidth() == 480)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetLightPause(), LANG_GetString(LSTR_SHORT_SECOND));
	} else
	if (LCDUI_GetScreenWidth() == 320)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetLightPause(), LANG_GetString(LSTR_SHORTEST_SECOND));
	}
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// lift height
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_LIFT_HEIGHT), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	sprintf(msg, "%0.1f %s", PFILE_GetLiftHeight(), LANG_GetString(LSTR_SHORT_MILLIMETERS));
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// lift speed
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_LIFT_SPEED), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	if (LCDUI_GetScreenWidth() == 480)
	{
	...
	if (LCDUI_GetScreenWidth() == 320)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetLiftSpeed(), LANG_GetString(LSTR_SHORTEST_MM_SEC));
	}
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// drop speed
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_DROP_SPEED), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	if (LCDUI_GetScreenWidth() == 480)
	{
	...
	if (LCDUI_GetScreenWidth() == 320)
	{
		sprintf(msg, "%0.1f %s", PFILE_GetDropSpeed(), LANG_GetString(LSTR_SHORTEST_MM_SEC));
	}
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// resin volume
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_RESIN_VOLUME), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	sprintf(msg, "%0.1f %s", PFILE_GetResinVolume(), LANG_GetString(LSTR_SHORT_MILLILITERS));
	LCDUI_DrawText(msg, LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	// resin volume
	ytop += fntheight + yinc;
	LCDUI_SetColor(LCDUI_RGB(0x000000));
	LCDUI_DrawText(LANG_GetString(LSTR_IND_LAYERS_SETTINGS), 0, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	LCDUI_SetColor(LCDUI_RGB(0x00496C));
	if (PFILE_GetIndLayerSettings())
		LCDUI_DrawText(LANG_GetString(LSTR_YES), LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
	else
		LCDUI_DrawText(LANG_GetString(LSTR_NO), LCDUI_TEXT_ALIGN_RIGHT, thisbtn->position.left + h_indent, ytop, thisbtn->position.right - h_indent, -1);
