...
void Planner::_buffer_steps(const int32_t (&target)[XYZE], float fr_mm_s, const uint8_t extruder) {
  ...
  block->step_event_count = block->steps[Z_AXIS];
  ...
  if (block->steps[Z_AXIS] < MIN_STEPS_PER_SEGMENT) {
    block->millimeters = FABS(delta_mm[E_AXIS]);
  }
  else {
    block->millimeters = SQRT(
        sq(sq(delta_mm[Z_AXIS])
    );
  }
...
void Planner::_set_position_mm(const float &c) {
  const int32_t nc = position[Z_AXIS] = LROUND(c * axis_steps_per_mm[Z_AXIS]));
  stepper.set_position(nc);
